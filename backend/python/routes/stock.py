...
async def get_stock_history(
    ...
    async def fetch():
        ...
        if not result.get("success") or result["data"] is None:
            ...
            return {
                "symbol": symbol,
                "history": [],
                "error": result.get("error", "No data found for symbol or date range"),
            }
        data = (
        ...
        missing = [
            col for col in expected_cols if data is None or col not in data.columns
        ]
    ...
    return await get_or_set_cache(
        f"history:{symbol}:{start}:{end}:{interval}", fetch, expire=600
    )
